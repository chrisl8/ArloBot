...
INSTALLING_ROS_DISTRO=foxy

# Run this straight off of github like this:
# bash <(wget -qO- --no-cache https://raw.githubusercontent.com/chrisl8/ArloBot/noetic/setup-foxy.sh)
...
# sudo docker run -ti -v $PWD:/home/user ubuntu:20.04 /bin/bash -c "/home/user/setup-foxy.sh"
#
...
printf "This script is part of my personal attempt to make Foxy work on this robot,\n"
...
if ! (locale | grep "en_US.UTF-8" >/dev/null); then
  printf "\n${YELLOW}[Setting locale to en_US.UTF-8 as recommended by Foxy]${NC}\n"
...
# TODO: These packages do not exist in Foxy:
#       E: Unable to locate package ros-foxy-move-base
#       E: Unable to locate package ros-foxy-twist-mux
#       E: Unable to locate package ros-foxy-rosbridge-server
...
# python3-argcomplete - The Foxy Install page recommends installing this.
...
# TODO: twist-mux does not exist for ROS2. Find out how to get it to Foxy,
...
# TODO: This does not exist in ROS 2. I think there is a replacement, but I'm not sure what it is.
...
if ! [[ ${WORKSTATION_INSTALL} == "y" ]]; then
  ...
  if [[ "${RESPONSE_TO_KINECT_QUERY}" == "y" ]] || [[ ${TRAVIS} == "true" ]]; then
    # Always test in Travis
    # TODO: Uncomment if they ever release freenect_stack for Foxy:
...
# TODO: Remove this if/when the apt package is released for foxy.
...
if ! [[ -d ~/dev_ws/src/teb_local_planner ]]; then
  git clone -b ${INSTALLING_ROS_DISTRO}-devel https://github.com/rst-tu-dortmund/teb_local_planner.git
else
  cd ~/dev_ws/src/teb_local_planner
  git checkout ${INSTALLING_ROS_DISTRO}-devel
  git pull
fi
...
if ! [[ ${WORKSTATION_INSTALL} == "y" ]]; then
  if [[ -d ~/dev_ws/src/ArloBot/mycroft-core ]]; then
    printf "\n${YELLOW}[Updating Mycroft]${NC}\n"
    cd ~/dev_ws/src/ArloBot/mycroft-core
  ...
  if [[ -d /opt/mycroft/skills ]]; then
    ...
    if ! [[ -L /opt/mycroft/skills/arlobot-robot-skill ]]; then
      ...
      ln -s "${HOME}/dev_ws/src/ArloBot/mycroft-arlobot-skill" arlobot-robot-skill
    fi
    if ! [[ -L /opt/mycroft/skills/arlobot-smalltalk-skill ]]; then
      cd /opt/mycroft/skills/
      ln -s "${HOME}/dev_ws/src/ArloBot/mycroft-smalltalk-skill" arlobot-smalltalk-skill
    fi
  fi
fi
