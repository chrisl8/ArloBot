...
//const int firtPINGsensorPIN = 5; // Which pin the first PING sensor is on - This is on the Quckstart Board now
//void pollPingSensors(void *par); // Use a cog to fill range variables with ping distances - This is just polling IR now.
...
*/
   ...
   cogstart(&pollPropBoard2, NULL, prop2stack, sizeof prop2stack);
	...
	//cog = cogstart(&pollPingSensors, NULL, pstack, sizeof pstack);
...
/* OUT OF COGS!
 * Moving This to the SafteOverride cog
 * Since the PING sensors are polled from the other board anyway.
void pollPingSensors(void *par) {
 while(1)                                    // Repeat indefinitely
  {
    // for(int i=0; i < numberOfPINGsensors; i++ ) {
    for(int i=0; i < numberOfIRonMC3208; i++ ) { // Change to numberOfIRonMC3208 as PINGs are on the QuickStart now.
        // pingArray[i] = ping_cm(firtPINGsensorPIN + i); // - PING sensors now on Quickstart Board
       ...
       //for (int j=0; j < numberOfIRonMC3208; j++) {
          ...
          pause(5); // Slight delay since IRs poll very quickly!
        //}
    }
  }
}
...
void pollPropBoard2(void *par) {
   //dprint(term, "propterm Start:");
   propterm = fdserial_open(propRXpin, propTXpin, 0, 115200);
   //dprint(term, "propterm Started.");
 pause(100); // Give the serial connection time to come up. Perhaps this is not required?
 const int bufferLength = 10; // Longer than longest possible sent line
  while (1) {
      pause(10); // This is the incoming rate limiter. Set it as low as you want. I also keeps the sensors quiet when this end is in an idle state.
      dprint(propterm, "i"); // Tell the other end we are alive, so it doesn't just spin pointlessly
      pause(10); // Give it time to respond so we don't "wrap" back to the rate limiting pause. TODO: Could be shorter?
      if(fdserial_rxReady(propterm) != 0) {
        ...
        if (buf[0] == 'p') {
          char *token;
				...
				token = strtok(NULL, delimiter);
				char *unconverted;
				int pingSensorNumber = strtod(token, &unconverted);
				token = strtok(NULL, delimiter);
          if (pingSensorNumber < numberOfPINGsensors) {
				  pingArray[pingSensorNumber] = strtod(token, &unconverted);
          }
        }
        fdserial_rxFlush(propterm); // Clear out any backlog, so we get up to date data!
...
while(1) {
    ...
    for (i = 0; i < numberOfIRonMC3208 - 1; i++) { // -1 to ignore the one in back for now.
      // TODO: Testing IR sensor polling here
      // The only concern is if it slows this cog down too much and allows for collisions
      irArray[i] = mcp3208_IR_cm(i);
    ...
    irArray[numberOfIRonMC3208 - 1] = mcp3208_IR_cm(numberOfIRonMC3208 - 1); // Rear Sensor
