...
find src/ -type f|grep -v package.xml|grep -v CMakeLists.txt
echo "Here are the file names without path:"
find src/ -type f|grep -v package.xml|grep -v CMakeLists.txt|awk -F'/' '{ print $NF }'
...
for i in $(find src/ -type f|grep -v package.xml|grep -v CMakeLists.txt|grep -v .stl|grep -v .pyc)
do
