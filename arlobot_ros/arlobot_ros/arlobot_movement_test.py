...
# TODO: https://github.com/tu-darmstadt-ros-pkg/hector_slam/issues/76
...
class ArlobotExplore(object):
    ...
    def Stop(self):
        ...
        self._MoveBaseClient.cancel_goals_at_and_before_time(
            rospy.Time.now()
        ...
        while (
            count < 2
        ):  # I'm not sure how many it takes, but more than one is best else sometimes it is missed.
            print(count)
            print(twist)
            pub.publish(twist)
            rospy.sleep(1)
    ...
    def Run(self):
        ...
        new_quaternion = tf2_ros.transformations.quaternion_multiply(
            [0.000, 0.000, -0.501, 0.865], quaternion_difference
        )
        ...
        while (
            count < 8
        ):  # 8 seconds seems about right at angular_speed = 1 to do just over 1 rotation normally
            print(count)
            print(twist)
            pub.publish(twist)
        ...
        try:
            get_exploration_path = rospy.ServiceProxy(
                "get_exploration_path", GetRobotTrajectory
            )
            ...
            result, resultText = self._movetoPositiononMap(
                explorePosition, exploreQuaternion, 20
            )
            """
            Timeout?
