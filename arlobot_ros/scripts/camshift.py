...
class CamShiftNode(ROS2OpenCV2):
    def __init__(self, node_name):
        ...
        cv.CreateTrackbar(
            "Threshold", "Parameters", self.threshold, 255, self.set_threshold
        )
    ...
    def process_image(self, cv_image):
        try:
            ...
            mask = cv2.inRange(
                hsv,
                np.array((0.0, self.smin, self.vmin)),
                np.array((180.0, 255.0, self.vmax)),
            )
            ...
            if self.hist is not None:
                ...
                ret, backproject = cv2.threshold(
                    backproject, self.threshold, 255, cv.CV_THRESH_TOZERO
                )
                ...
                self.track_box, self.track_window = cv2.CamShift(
                    backproject, self.track_window, term_crit
                )
    ...
    def hue_histogram_as_image(self, hist):
        ...
        cv.ConvertScale(mybins, mybins, 255.0 / hi)
