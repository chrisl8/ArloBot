...
# This should follow the official ROS install instructions closely.
# That is why there is a separate section for extra packages that I need for Arlo.
if ! (dpkg -s ros-kinetic-desktop | grep "Status: install ok installed" &>/dev/null); then
...
sudo apt install -y vlc-nox zbar-tools python-qrtools qtqr ros-kinetic-rtabmap-ros git
...
cd ~/catkin_ws/src
if ! [ -d ~/catkin_ws/src/hector_slam ]; then
...
cd ~/catkin_ws/src
if ! [ -d ~/catkin_ws/src/hector_navigation ]; then
...
cd ~/catkin_ws/src
if ! [ -f ${HOME}/Desktop/listen2robot.desktop ]; then
