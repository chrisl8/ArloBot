...
if (command -v jq >/dev/null) && [[ $(jq '.cloudServer.exists' "${HOME}/.arlobot/personalDataForBehavior.json") == true ]]; then
  ...
  if ! node "${HOME}/dev_ws/src/ArloBot/node/addRobotIpToEtcHosts.js" >/dev/null; then
    ...
    ROBOT_DATA=$(node "${HOME}/dev_ws/src/ArloBot/node/getRobotDataFromWeb.js" json)
    export ROBOT_DATA
    sudo "${HOME}/.nvm/versions/node/$(nvm version)/bin/node" "${HOME}/dev_ws/src/ArloBot/node/addRobotIpToEtcHosts.js" "${ROBOT_DATA}"
  fi
fi
