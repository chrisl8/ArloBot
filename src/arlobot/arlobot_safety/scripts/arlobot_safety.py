...
class ArlobotSafety(object):
    def __init__(self):
        ...
        self.acPower = (
            True
        )  # Status of whether laptop is plugged in or not. We assume 1, connected, to start with because that is the most restrictive state.
    ...
    def Run(self):
        while not rospy.is_shutdown():
            ...
            ):  # If arlobot_bringup is running
                checkAC = rospy.get_param(
                    "/arlobot/monitorACconnection"
                )  # Use parameter from arlobot_bringup to decide if we should monitor AC or not
            else:
