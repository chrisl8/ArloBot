...
class PropellerSerialInterface(object):
    ...
    def SendToPropellerOverSerial(self, packetType, data, debug=False):
        ...
        if len(dataToSend) > self._maximumSerialDataLength:
            ...
            self._serialWriteInProgress = False
            return False
        txLen = chr(len(dataToSend))
        adjSendStr = (
            chr(self._serialDataStartMarker)
            + txLen
            + dataType
            + dataToSend
            + chr(self._serialDataEndMarker)
        )
        self._SerialDataGateway.Write(adjSendStr)
    ...
    def _EncodeHighBytes(self, inStr):
        ...
        for n in range(0, s):
            ...
            if x >= self._serialDataSpecialByte:
                outStr = outStr + chr(self._serialDataSpecialByte)
                outStr = outStr + chr(x - self._serialDataSpecialByte)
            else:
                outStr = outStr + chr(x)

        return outStr

    def _DecodeHighBytes(self, inStr):

        outStr = ""
        n = 0

        while n < len(inStr):
            if ord(inStr[n]) == self._serialDataSpecialByte:
                n += 1
                # Avoid crashing Python on very bad data
                if self._serialDataSpecialByte + ord(inStr[n]) < 256:
                    x = chr(self._serialDataSpecialByte + ord(inStr[n]))
                else:
                    x = "-"
            else:
                x = inStr[n]
    ...
    def HandleSerialDataInput(self, receivedCharacter):
        # This kind of mirrors PropellerCodeForArloBot/ReceiveSerialData.h
        # self._output(BytesToString(receivedCharacter) + "\n")
        if self._serialReceiveInProgress:
            if ord(receivedCharacter) != self._serialDataEndMarker:
                if (
                    self._serialReceiveDataLength == 0
                    ...
                    self._serialReceiveDataLength = ord(receivedCharacter)
                    if (
                    ...
                    self._serialReceiveDataType = chr(ord(receivedCharacter))
                else:
                    ...
                    if self._serialReceiveIndex < self._serialReceiveDataLength:
                        self._serialReceiveBuffer = (
                            self._serialReceiveBuffer + receivedCharacter
                        )
                        # self._output(BytesToString(serialReceiveBuffer) + "\n")
                ...
                if not self._serialReceiveError:
                    ...
                    if self._serialReceiveDataLength == self._serialDataSpecialByte:
                        ...
                        # is started.
                        self._output("Received ERROR: " + self._serialReceiveBuffer)
                        self._serialReceiveError = True
                ...
                if not self._serialReceiveError:
                    ...
                    receivedChecksum = ord(self._serialReceiveBuffer[-1])
        ...
        elif ord(receivedCharacter) == self._serialDataStartMarker:
    ...
    def _AddCheckSum(self, dataToSend):
        # https://henryforceblog.wordpress.com/2015/03/12/designing-a-communication-protocol-using-arduinos-serial-library/
        dataSum = 0
        for element in dataToSend:
            dataSum = dataSum + ord(element)
        dataCheckSum = dataSum & 0xFF
        outStr = dataToSend + chr(dataCheckSum)
