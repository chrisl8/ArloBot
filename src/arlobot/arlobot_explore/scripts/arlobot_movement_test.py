...
#TODO: Test for robot movement before sending it places! It could get really goofy to send to -90 from it's position ten seconds ago!
    ...
    def Run(self):
        ...
        if not rospy.is_shutdown():
            before_time =  rospy.Time.now()
            self._MoveBaseClient.send_goal(goal)
            result = -1
            result = self._MoveBaseClient.wait_for_result(rospy.Duration(30))
            print result
            result = self._MoveBaseClient.get_state()
            print result
            if result < 3:
                ...
                result = self._MoveBaseClient.get_state()
                print result
            total_time = rospy.Time.now() - before_time
            print total_time
        #self._MoveBaseClient.send_goal(goal)
        #rospy.loginfo("Waiting for response . . .");
        #self._MoveBaseClient.wait_for_result()
        ...
        if not rospy.is_shutdown():
            self._MoveBaseClient.send_goal(goal)
            while self._MoveBaseClient.get_state() < 3:
                print self._MoveBaseClient.get_state()
                rospy.sleep(1)
        #self._MoveBaseClient.send_goal(goal)
        #rospy.loginfo("Waiting for response . . .");
        #self._MoveBaseClient.wait_for_result()
